...
class AGranFlat:
    def __init__(self,Lx = 100.0,Ly=100.0,AR = 1.5,r0 = 1., rb = 0.3,rho=0.25, r_tr = 8, T = 10,factor = 0.1, v0 = 1.2, eta = 1500,mua = 0.0007, mup = 0.001, mur = 0.0001,mu_tr = 0.0001,trN = 1, k = 200,v_drag=1, mode='drag',lfrac = 0.8,pfrac = 0.2,Nrelax = 20000):
...
#         TAU += 3*self.eta*np.sqrt(self.dt)*np.random.uniform(-np.pi, np.pi, size=self.N)/self.mur
    ...
    def relax(self):
        ...
        dist = treeall.sparse_distance_matrix(tree, max_distance=self.r0*2*2**(1/6),output_type='coo_matrix')
        while (len(dist.col)>self.N):
            ...
            tree = cKDTree(self.pos,boxsize=[self.Lx,self.Ly])
            treeall = cKDTree(np.concatenate([self.pos,self.body_tr]),boxsize=[self.Lx,self.Ly])
            dist = treeall.sparse_distance_matrix(tree, max_distance=self.r0*1.9*2**(1/6),output_type='coo_matrix')
